...
2. 새 파일을 만들기 전에 `docs/followup/active/*.md`를 확인한다.
   - active 파일이 1개 있고 현재 작업과 같은 축이면 새 파일을 만들지 않고 같은 파일을 유지한다.
...
4. 이미 active 파일이 있으면 전체를 다시 쓰지 않고, 다음 세션 복구에 필요한 섹션만 현재 상태 기준으로 갱신한다. 전체 구조 재작성은 새 작업 축을 열거나 active 파일이 없을 때만 허용한다.
