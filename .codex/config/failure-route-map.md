...
- 자동 라우팅은 기존에 정의된 실패 축 후보만 반환한다.
- 메인 에이전트가 반환된 실패 정보와 후보를 확인해 runbook 게이트와 실패 축을 확정한다.
...
- `.codex/scripts/verify-and-retry.sh`는 전달받은 실패 정보로 기존 실패 축 후보를 반환한다.
- 메인이 runbook 게이트와 실패 축을 확정한 경우에만 해당 runbook에 실패 케이스를 기록한다.
