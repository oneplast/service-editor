...
2. 읽어야 할 기준 문서가 2개 이상으로 늘어날 때
3. 구현/문서 반영/검증처럼 작업 단계가 바뀔 때
4. 컨텍스트가 길어져 다시 읽기 비용이 커질 때
5. 작업이 끝나지 않은 채 턴을 마칠 때
...
- 세션 재개 직후 관련 작업 축 runbook이 있으면, followup으로 현재 상태를 맞춘 뒤 그 runbook의 `작업 전 체크포인트`도 함께 다시 본다.
