...
- 같은 작업 축의 active 파일이 이미 있으면 새 파일을 만들거나 전체를 다시 쓰지 않고, 다음 세션 복구에 필요한 섹션만 현재 상태로 갱신한다.
- 새 사실이 생기면 계속 덧붙이지 말고, 완료된 내용은 줄이고 최신 상태만 남긴다.
- 관련 없는 여러 작업을 한 파일에 섞지 않는다.
- 새 active 파일을 만들기 전에는 기존 `docs/followup/active/*.md`를 먼저 확인하고, 같은 작업 축이면 기존 파일을 유지한다.
