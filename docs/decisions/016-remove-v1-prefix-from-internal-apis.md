...
- editor-service는 gateway 뒤의 내부 서비스로 운영된다.
- 기존에는 내부 컨트롤러 경로가 `/v1`를 직접 포함하고 있어 외부 버전 정책과 내부 구현이 결합되어 있었다.

## 결정

- editor-service 내부 API 경로에서 `/v1` 프리픽스를 제거한다.
- 내부 표준 경로는 `/workspaces/**`, `/documents/**`, `/admin/**`를 사용한다.
- gateway는 외부 클라이언트에 `/v1/**`를 제공하고, editor-service 전달 전에 `/v1` 프리픽스를 제거(rewrite)한다.
...
- [2026-03-27-internal-api-path-without-v1-review.md](https://github.com/oneplast/service-editor/blob/dev/docs/discussions/2026-03-27-internal-api-path-without-v1-review.md)
