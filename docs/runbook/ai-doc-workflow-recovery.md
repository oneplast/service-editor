...
- 문서 읽기 순서나 경로 판단이 흔들리면 `검증/자기점검 때 볼 분기`에서 가까운 항목 하나를 먼저 고른다.
- 세션 재개 직후 현재 작업과 다음 액션이 흐려졌다면 followup의 `현재 상태`와 `다음 작업`을 먼저 맞춘다.
- 이미 같은 문서 작업 실수가 반복됐다면 `반복 실수/실패 사례`에서 가까운 사례만 먼저 확인한다.
- 문서 체계나 규칙 변경 뒤 검증이 필요하면 `실패 후 복구 순서`의 거버넌스 검증 단계부터 확인한다.
...
5. 문서 체계나 규칙을 바꿨다면 확정한 변경 성격과 scope를 넘겨 `.codex/scripts/check-doc-governance.sh`를 실행한다.
